...
typedef struct
{
    uint128_t *key_2 = NULL;
    uint128_t *key_3 = NULL;

    // optional (only for MSK)
    uint128_t correction_2;
    uint128_t *corrections_3 = NULL;
    uint8_t *delta = NULL;
} Key;

typedef struct
{
    uint128_t *cache_2 = NULL;
    uint128_t *cache_3 = NULL;
#ifdef AVX
    __m512i *cache_2_avx;
    __m512i *cache_3_avx;
#endif
...
void key_gen(const PublicParams *pp, Key *msk)
{
    ...
    msk->key_2 = (uint128_t *)malloc(sizeof(uint128_t) * pp->key_len);
    msk->key_3 = (uint128_t *)malloc(sizeof(uint128_t) * pp->key_len * 2);

    RAND_bytes((uint8_t *)msk->key_2, sizeof(uint128_t) * pp->key_len);

    // Generate unbiased values mod 3 via rejection sampling
    uint8_t *rand_3 = (uint8_t *)malloc(sizeof(uint8_t) * pp->key_len * RING_DIM);
    ...
    msk->delta = (uint8_t *)malloc(sizeof(uint8_t) * RING_DIM);
    ...
    const size_t num_ots)
{
    ...
    uint128_t *hash_in = (uint128_t *)malloc(sizeof(uint128_t) * num_ots * 6 * 3);
    uint128_t *hash_out = (uint128_t *)malloc(sizeof(uint128_t) * num_ots * 6 * 3);
    ...
    const size_t num_ots)
{
    ...
    uint128_t *hash_in = (uint128_t *)malloc(sizeof(uint128_t) * num_ots * 3);
    uint128_t *hash_out = (uint128_t *)malloc(sizeof(uint128_t) * num_ots * 3);
...
void compute_correction_terms(
    ...
    const uint8_t *delta)
{
    msk->corrections_3 = (uint128_t *)malloc(sizeof(uint128_t) * 6 * 2);
