...
// something is wrong with this function, perhaps the CRH, perhaps the tmp layout
// using expand_4to8 instead
...
class TwoKeyPRP { public:
	...
	void node_expand_4to8(block *children, block *parent) {
		...
		tmp[7] = children[7] = otho(parent[3] ^ one_block);
		tmp[3] = children[6] = otho(parent[3] ^ zero_block);
		tmp[6] = children[5] = otho(parent[2] ^ one_block);
		tmp[2] = children[4] = otho(parent[2] ^ zero_block);
		tmp[5] = children[3] = otho(parent[1] ^ one_block);
		tmp[1] = children[2] = otho(parent[1] ^ zero_block);
		tmp[4] = children[1] = otho(parent[0] ^ one_block);
		tmp[0] = children[0] = otho(parent[0] ^ zero_block);
